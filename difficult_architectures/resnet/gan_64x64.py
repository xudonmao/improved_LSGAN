...
DATA_DIR = './bedroom_png_64/'
if len(DATA_DIR) == 0:
    raise Exception('Please specify path to data directory in gan_64x64.py!')

MODE = 'lsgan-gp' # dcgan, wgan, wgan-gp, lsgan
DIM = 64 # Model dimensionality
CRITIC_ITERS = 2 # How many iterations to train the critic for
N_GPUS = 1 # Number of GPUs
BATCH_SIZE = 64 # Batch size. Must be a multiple of N_GPUS
ITERS = 500000 # How many iterations to train for
LAMBDA = 150 # Gradient penalty lambda hyperparameter
CC = 30 # Gradient penalty lambda hyperparameter
OUTPUT_DIM = 64*64*3 # Number of pixels in each iamge

lib.print_model_settings(locals().copy())

def get_perturbed_batch(minibatch):
    return minibatch + CC * np.random.random(minibatch.shape)
...
def Normalize(name, axes, inputs):
    if ('Discriminator' in name) and (MODE == 'lsgan-gp'):
        if axes != [0,2,3]:
...
with tf.Session(config=config) as session:
    ...
    for device_index, (device, real_data_conv) in enumerate(zip(DEVICES, split_real_data_conv)):
        with tf.device(device):
            ...
            if MODE == 'wgan':
                ...
                gen_cost = tf.reduce_mean(tf.nn.l2_loss(disc_fake - tf.ones_like(disc_fake))) 

                disc_cost = tf.reduce_mean(tf.nn.l2_loss(disc_real - tf.ones_like(disc_real))) 

                disc_cost += tf.reduce_mean(tf.nn.l2_loss(disc_fake - tf.zeros_like(disc_fake))) 
                ...
                real_data_perturbed = get_perturbed_batch(real_data)
                differences = real_data_perturbed - real_data
    ...
    #lib.save_images.save_images(_x_r.reshape((BATCH_SIZE/N_GPUS, 3, 64, 64)), 'samples/samples_groundtruth.png')
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(disc_iters):
          #_data = gen.next()
            _data = session.run(data_set)
            _data = _data.transpose(0,3,1,2)
        ...
        if iteration % 10000 == 0:
            saver.save(session, '{0}/{1}.model'.format('checkpoint', iteration))
