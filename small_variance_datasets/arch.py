...
def discriminator_digit(image, reuse=False, for_G=False):
  with tf.variable_scope('discriminator', reuse=reuse): 
  
    df_dim = 64

    h0 = lrelu(conv2d(image, 20, name='d_h0_conv'))
    h1 = lrelu(d_bn1(conv2d(h0, 50, name='d_h1_conv')))
    h1 = tf.reshape(h1, [FLAGS.batch_size, -1])
    h2 = lrelu(d_bn2(linear(h1, 500, 'd_h2_lin')))
    h3 = linear(h2, 1, 'd_h3_lin')

    return h3
def generator_digit(z, reuse=False, bn_train=True):
  with tf.variable_scope('generator', reuse=reuse):

    s = FLAGS.output_size
    s2, s4, s8, s16 = int(s/2), int(s/4), int(s/8), int(s/16)

    h0 = tf.nn.relu(g_bn0(linear(z, 64*8*4*4, 'g_h0_lin'), train=bn_train), name='g_h0_relu')
    h0 = tf.reshape(h0, [FLAGS.batch_size, 4, 4, 512])
    h1 = tf.nn.relu(g_bn1(deconv2d(h0,[FLAGS.batch_size, s4, s4, 256], name='g_h1'), train=bn_train),name='g_h1_relu')
    h2 = tf.nn.relu(g_bn2(deconv2d(h1,[FLAGS.batch_size, s2, s2, 128], name='g_h2'), train=bn_train),name='g_h2_relu')
    h3 = tf.nn.tanh(deconv2d(h2, [FLAGS.batch_size, s, s, FLAGS.c_dim], name='g_h3'), name='g_h3_sigmoid')
